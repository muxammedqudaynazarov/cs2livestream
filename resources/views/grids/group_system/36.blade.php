    ...
    <div class="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div
            ...
            <div>
                <div
                    ...
                    <i class="fa-solid fa-layer-group text-blue-400"></i> Topar basqıshı
                </div>
                <h1 class="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">Tiykarǵı toparlar</h1>
                <p class="text-sm font-medium text-slate-500 mt-1">
                    Hár toparda 6 komanda. 1-2-orınlar hám 4 eń jaqsı nátiyje kórsetken
                    3-orınlar pley-off (1/8 final) basqıshına shıǵadı.
                </p>
            </div>
        </div>
        @php
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mb-12">
            @foreach($groups as $group)
                <div
                    class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden flex flex-col transform hover:-translate-y-1 transition-all duration-300 hover:shadow-md">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left border-collapse">
                            ...
                            @for($i = 1; $i <= 6; $i++)
                                <tr class="border-b border-slate-50 last:border-0 hover:bg-slate-50/50 transition-colors {{ $i <= 2 ? 'bg-emerald-50/30' : ($i == 3 ? 'bg-blue-50/10' : '') }}">
                                    ...
                                    <td class="px-2 py-2.5 text-center relative">
                                        ...
                                        @elseif($i == 3)
                                            <div
                                                class="absolute left-0 top-0 bottom-0 w-1 bg-blue-400 border-r border-white border-dashed"></div>
                                        @endif
                                        <span
                                            class="{{ $i <= 2 ? 'text-emerald-600 font-bold' : ($i == 3 ? 'text-blue-600 font-bold' : 'text-slate-400') }}">
                                            {{ $i }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="px-2 py-2.5 text-center font-bold text-emerald-600">{{ 5 - $i >= 0 ? 5 - $i : 0 }}</td>
                                    <td class="px-2 py-2.5 text-center text-rose-500">{{ $i - 1 }}</td>

                                    <td class="px-4 py-2.5 text-center font-black text-slate-900 bg-slate-50/50">{{ (5 - $i >= 0 ? 5 - $i : 0) * 3 }}</td>
                                </tr>
                            @endfor
                            </tbody>
                        </table>
        ...
        <div class="pt-8 border-t border-slate-200">
            ...
            <div class="flex gap-2 overflow-x-auto pb-4 custom-scrollbar mb-6">
                <button
                    class="px-5 py-2 rounded-lg bg-slate-900 text-white font-bold text-sm shadow-sm whitespace-nowrap transform scale-105 transition-transform">
                    1-tur
                </button>
                <button
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                @foreach($groups as $group)
                    <div class="flex flex-col gap-3 bg-slate-50/50 p-4 rounded-2xl border border-slate-100">
                        ...
                        @for($match=1; $match<=3; $match++)
                            <div
                                ...
                                <div class="flex justify-between items-center px-4 py-3 border-b border-slate-50">
                                    ...
                                    <span
                                        class="text-sm font-black text-slate-900 bg-slate-100 px-2.5 py-1 rounded border border-slate-200">13</span>
                                </div>

                                <div class="flex justify-between items-center px-4 py-3 bg-slate-50/80">
                                    <span
                                        class="text-sm font-bold text-slate-500">Komanda {{ $group }}{{ $match*2 }}</span>
                                    <span class="text-sm font-bold text-slate-400 px-2.5 py-1">8</span>
                                </div>

                                <div
                                    class="bg-white border-t border-slate-100 px-4 py-2.5 flex justify-between items-center">
                                    <span class="text-[10px] font-bold text-slate-400 uppercase"><i
                                            class="fa-regular fa-clock mr-1"></i> Búgin, 18:00</span>
                                    <span
                                        class="text-[10px] font-bold text-blue-500 uppercase tracking-wider bg-blue-50 px-2 py-0.5 rounded">BO1</span>
                                </div>
                            </div>
                        @endfor
                    </div>
